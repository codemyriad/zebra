...
async function _fetchConversations(sessionToken: string, offset = 0): Promise<ConversationsQueryResponse> {
  const res = await fetch(`https://chatgpt.com/backend-api/conversations?offset=${offset}`, {
    "headers": {
...
function _pickFromConversation(item: ConversationItemRaw): Omit<Conversation, "messages"> {
  return {
    id: item.id,
    title: item.title,
...
function _processConversations(items: ConversationItemRaw[]): Omit<Conversation, "messages">[] {
  return items.map(_pickFromConversation)
}

async function getConversations(sessionToken: string): Promise<Omit<Conversation, "messages">[]> {
  ...
  const res = await Promise.all(Array(totalRequests).fill(null).map((_, i) => _fetchConversations(sessionToken, i * 50)))
...
async function _fetchMessages(sessionToken: string, conversationId: string): Promise<MessageContentRaw[]> {
  const res = await fetch(`https://chatgpt.com/backend-api/conversation/${conversationId}`, {
    "headers": {
      "authorization": `Bearer ${sessionToken}`,
    },
  })
  ...
  return Object.values(json.mapping).map(({ message }) => message).filter(Boolean) as MessageContentRaw[]
}
...
function processMessages(messages: MessageContentRaw[]): Message[] {
  return messages.filter(messageFilter).map(pickFromMessage).filter(({ content }) => Boolean(content))
}

async function getMessages(sessionToken: string, conversationId: string): Promise<Message[]> {
...
export async function getConversationHistory(): Promise<Conversation[]> {
  ...
  return await Promise.all(conversations.map(async (c) => ({ ...c, messages: await getMessages(sessionToken, c.id) })))
}
